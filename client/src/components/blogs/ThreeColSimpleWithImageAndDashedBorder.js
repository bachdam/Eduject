...
const Details = tw.div`p-6 rounded border-2 border-t-0 rounded-t-none border-dashed border-teal-300 flex-1 flex flex-col items-center text-center lg:block lg:text-left`;
...
const Link = styled(PrimaryButtonBase).attrs({ as: "a" })`
  ${tw`inline-block mt-4 text-sm font-semibold bg-teal-600`}
`;
...
export default ({
  subheading = <span tw="text-yellow-500">Blog</span>,
  heading = (
    <>
      We Love <span tw="text-teal-500">Writing ✍️</span>
    </>
  ),
  description = "Some amazing blog posts that are written by even more amazing people of Eduject.",
}) => {
  const blogPosts = [
    {
      imageSrc:
